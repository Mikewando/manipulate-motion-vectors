...
fn formatHelper(comptime T: type, comptime bits_per_sample: u8) type {
    return struct {
        fn drawVector(
            ...
            var short_len: i32 = undefined;
            var long_len: i32 = undefined;
        ...
        pub fn getFrame(n: c_int, activation_reason: vs.ActivationReason, instance_data: ?*anyopaque, frame_data: ?*?*anyopaque, frame_ctx: ?*vs.FrameContext, core: ?*vs.Core, vsapi: ?*const vs.API) callconv(.c) ?*const vs.Frame {
            ...
            if (activation_reason == .Initial) {
                ...
                if (d.use_sc_props) {
                    ...
                    const scene_change = if (backwards)
                        src_props.getBool("_SceneChangeNext") orelse false
                    else
                        src_props.getBool("_SceneChangePrev") orelse false;
                ...
                const vector_data = src_props.getIntArray("MVUtensilsAnalysisVectors") orelse {
                    // No usable vectors
                    return dst.frame;
                };
                ...
                for (vector_data, 0..) |vectors, block_index| {
